...
def createDB():  # creates the database if it doesn't exist yet.
    ...
    for x in cursor:
        if str(x) == "('polls_floor',)":  # if a floor equals this, it exists
            ...
            buildingexists = True
        elif str(x) == "('polls_rooms',)":
            roomexists = True
    if not floorexists:  # if the floor table doesn't exist, create it!
        try:
    ...
    if not roomexists:
        try:
            ...
            print("ERROR: " + str(e))
    if not floorexists:  # if the floor table was just created, add the values
        try:
